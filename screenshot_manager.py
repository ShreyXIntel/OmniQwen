...
Updated for new folder structure and timing system.
"""
import os
import logging
import time
...
class ScreenshotManager:
    ...
    def take_screenshot(self, save_immediately: bool = True, custom_name: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            if save_immediately:
                ...
                # Save to raw screenshots directory (using new structure)
    ...
    def cleanup_old_screenshots(self, max_age_hours: float = 24.0, 
        ...
        try:
            ...
            # Get all screenshot files from raw screenshots directory
                ...
                for filename in sorted(os.listdir(raw_screenshots_dir)):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        if os.path.isfile(filepath):
                            ...
                            if info:
                                summary["raw_screenshots"].append(info)
            
            # Get info for analyzed screenshots
            analyzed_screenshots_dir = self.directories["analyzed_screenshots"]
            if os.path.exists(analyzed_screenshots_dir):
                for filename in sorted(os.listdir(analyzed_screenshots_dir)):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                        filepath = os.path.join(analyzed_screenshots_dir, filename)
                        if os.path.isfile(filepath):
                            info = self.get_screenshot_info(filepath)
                            if info:
                                summary["analyzed_screenshots"].append(info)
            
            # Get info for benchmark result screenshots
            benchmark_results_dir = self.directories["benchmark_results"]
            if os.path.exists(benchmark_results_dir):
                for filename in sorted(os.listdir(benchmark_results_dir)):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                        filepath = os.path.join(benchmark_results_dir, filename)
                        if os.path.isfile(filepath):
                            info = self.get_screenshot_info(filepath)
                            if info:
                                summary["benchmark_results"].append(info)
            
            # Calculate total sizes
            total_raw_size_mb = sum(info["size_mb"] for info in summary["raw_screenshots"])
            total_analyzed_size_mb = sum(info["size_mb"] for info in summary["analyzed_screenshots"])
            total_results_size_mb = sum(info["size_mb"] for info in summary["benchmark_results"])
            
            summary["total_raw_size_mb"] = round(total_raw_size_mb, 2)
            summary["total_analyzed_size_mb"] = round(total_analyzed_size_mb, 2)
            summary["total_results_size_mb"] = round(total_results_size_mb, 2)
            summary["total_size_mb"] = round(total_raw_size_mb + total_analyzed_size_mb + total_results_size_mb, 2)
            
            # Count screenshots
            summary["raw_screenshots_count"] = len(summary["raw_screenshots"])
            summary["analyzed_screenshots_count"] = len(summary["analyzed_screenshots"])
            summary["benchmark_results_count"] = len(summary["benchmark_results"])
    ...
    def save_screenshot_summary(self, output_path: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = os.path.join(self.directories["runtime_logs"], filename)
